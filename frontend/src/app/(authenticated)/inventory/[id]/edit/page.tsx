...
export default function EditItemPage() {
  ...
  const [form, setForm] = useState({
    ...
    type: item?.type ?? "",
    code: item?.code ?? "",
    category: item?.category ?? "2026",
    unitPricePc: item?.unitPricePc?.toString() ?? "",
    unitPriceCtn: item?.unitPriceCtn?.toString() ?? "",
    lowStockThresholdPc: item?.lowStockThresholdPc?.toString() ?? "",
    lowStockThresholdCtn: item?.lowStockThresholdCtn?.toString() ?? "",
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setSaving(true);
    setTimeout(() => {
      alert("Item would be updated in Firestore (dev mode)");
      setSaving(false);
    }, 500);
  };
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <select id="category" value={form.category} onChange={(e) => updateField("category", e.target.value)} className="w-full h-9 rounded-2xl border border-transparent bg-canvas px-3 text-sm outline-none transition-colors focus-visible:border-hairline focus-visible:bg-paper focus-visible:ring-2 focus-visible:ring-hairline/40">
              ...
              <div className="space-y-2">
                ...
                <Input id="unitPricePc" type="number" min={0} value={form.unitPricePc} onChange={(e) => updateField("unitPricePc", e.target.value)} required />
              </div>
              <div className="space-y-2">
                <Label htmlFor="unitPriceCtn">Unit Price (CTN) *</Label>
                <Input id="unitPriceCtn" type="number" min={0} value={form.unitPriceCtn} onChange={(e) => updateField("unitPriceCtn", e.target.value)} required />
              </div>
              <div className="space-y-2">
                <Label htmlFor="lowStockPc">Low Stock Threshold (PC)</Label>
                <Input id="lowStockPc" type="number" min={0} value={form.lowStockThresholdPc} onChange={(e) => updateField("lowStockThresholdPc", e.target.value)} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="lowStockCtn">Low Stock Threshold (CTN)</Label>
                <Input id="lowStockCtn" type="number" min={0} value={form.lowStockThresholdCtn} onChange={(e) => updateField("lowStockThresholdCtn", e.target.value)} />
              </div>
            </div>
